...
const allowedOrigins = [
    "http://localhost:5173",
    "https://atelier-route-du-rhum-project-seven.vercel.app"
];

app.use(cors({
    origin: function (origin, callback) {
        // On autorise si l'origine est dans la liste ou si c'est une requête sans origine (ex: Postman)
        if (!origin || allowedOrigins.indexOf(origin) !== -1) {
            callback(null, true);
        } else {
            callback(new Error("Accès refusé par le protocole CORS de l'Établissement"));
        }
    },
    credentials: true,
