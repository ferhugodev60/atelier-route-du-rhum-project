...
export default function AdminWorkshops() {
    ...
    // 🏺 Filtrage des séances par typologie
    const { discovery, conception } = useMemo(() => {
        return {
            discovery: workshops.find(ws => ws.level === 0 || ws.title.toLowerCase().includes('découverte')),
            conception: workshops.filter(ws => ws.level > 0 && !ws.title.toLowerCase().includes('découverte'))
        };
    }, [workshops]);
    ...
    return (
        <section className="space-y-20 font-sans pb-20">
            <header className="border-b border-rhum-gold/10 pb-8">
                <h2 className="text-3xl font-serif text-white uppercase tracking-tight">Gestion des Formations</h2>
                <p className="text-[10px] text-rhum-gold/50 uppercase tracking-[0.4em] mt-2 font-black">Architecture des services</p>
            </header>

            {/* --- SECTION 1 : SÉANCE D'INITIATION (Unique) --- */}
            {discovery && (
                    ...
                    <div className="relative bg-[#0a1a14] border border-rhum-gold/20 p-8 md:p-12 flex flex-col lg:flex-row gap-12 items-center overflow-hidden group">
                        ...
                        <div className="flex-1 space-y-6">
                            ...
                            <div className="pt-8 flex gap-12 border-t border-white/5 items-center">
                                ...
                                <div className="space-y-1">
                                    <p className="text-[8px] text-white/20 uppercase font-black">Calendrier</p>
            ...
            {/* --- SECTION 2 : CURSUS DE CONCEPTION (Progression) --- */}
            <div className="space-y-10">
                ...
                <div className="grid grid-cols-1 gap-6">
                    {conception.map((ws) => (
                        ...
                        >
                            {/* Design inspiré de image_e35bc7.png */}
                            ...
                            <div className="mt-8 md:mt-0 flex items-center gap-12 border-t md:border-t-0 md:border-l border-white/5 pt-6 md:pt-0 md:pl-12">
                                <div className="text-right">
                                    <p className="text-white font-serif text-lg tracking-tight">Session : {ws.price}€</p>
                                    <p className="text-[9px] text-rhum-gold/40 uppercase tracking-[0.2em] mt-1">{ws.availability || 'Sur Demande'}</p>
                                </div>
                                <button
                                    onClick={() => { setSelectedWorkshop(ws); setIsEditOpen(true); }}
                                    className="p-3 text-rhum-gold/20 hover:text-rhum-gold transition-colors"
                                >
                                    <Edit3 size={18} />
                                </button>
                            </div>
