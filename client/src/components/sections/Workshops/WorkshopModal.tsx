...
export default function WorkshopModal({ detail, onClose, onReserve }: WorkshopModalProps) {
    ...
    const renderFooterButton = () => {
        ...
        if (isLocked) {
            return (
                <button disabled className="w-full bg-black/40 text-white/20 py-4 font-black uppercase tracking-[0.3em] text-[10px] rounded-sm cursor-not-allowed">
                    Validez le niveau {currentLevel + 1} pour progresser
                </button>
            );
        }
        ...
        return (
            ...
            >
                {isMastered ? 'Réserver à nouveau' : 'Réserver dès maintenant'}
            </button>
        );
    };
    ...
    return createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center overflow-hidden font-sans">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-[#0a1a14] w-full h-full md:h-auto md:max-h-[92dvh] md:max-w-6xl flex flex-col md:flex-row z-10 overflow-hidden shadow-2xl border-white/5">
                <button onClick={onClose} className="absolute top-4 right-6 text-rhum-gold/40 hover:text-white transition-colors z-[100] text-4xl font-light">&times;</button>

                <div className="w-full h-[200px] md:h-auto md:w-[40%] relative overflow-hidden bg-black border-r border-white/5">
                    <img
                        src={detail.image}
                        alt={detail.title}
                        className={`w-full h-full object-cover opacity-60 transition-all duration-700 ${isLocked ? 'grayscale scale-105' : 'scale-100'}`}
                    />
                </div>

                <div className="flex-1 flex flex-col bg-[#0a1a14]">
                    <header className="p-6 md:p-8">
                        <span className="text-rhum-gold text-[9px] uppercase tracking-[0.4em] font-black mb-2 block opacity-50">
                            {isMastered ? 'Maîtrise Acquise' : `Niveau ${detail.level}`}
                        </span>
                        <h5 className="text-2xl md:text-4xl font-serif text-white mb-2 uppercase tracking-tighter">{detail.title}</h5>
                        <p className="text-rhum-gold text-base font-serif">{detail.description}</p>
                    </header>

                    <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 custom-scrollbar">
                        <div className="p-4 bg-white/[0.03] border-l-2 border-rhum-gold/40">
                            <p className="text-[9px] uppercase tracking-[0.3em] text-rhum-gold mb-1.5 font-black">Format de séance :</p>
                            <p className="text-white/90 text-sm">{detail.format}</p>
                        </div>
                        {detail.availability && (
                            <div className="p-4 bg-white/[0.03] border-l-2 border-rhum-gold/40">
                                <p className="text-[9px] uppercase tracking-[0.3em] text-rhum-gold mb-1.5 font-black">Disponibilités :</p>
                                <p className="text-white/90 text-sm leading-relaxed">{detail.availability}</p>
                            </div>
                        )}
                        <p className="text-rhum-cream/60 text-base leading-relaxed font-serif">"{detail.quote}"</p>
                    </div>
