...
export default function WorkshopModal({ detail, onClose, onReserve }: WorkshopModalProps) {
    ...
    const handleReservation = () => {
        // 🏺 On transmet le titre et le niveau spécifique (1, 2, 3 ou 4)
        onReserve({
            ...detail,
            cartId: `${detail.id}-${Date.now()}`,
            name: detail.title, // Scelle le nom blanc
            level: detail.level, // Scelle le label Conception
            quantity: 1
        });
        onClose();
    };
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-[#0a1a14] w-full h-full md:h-auto md:max-h-[92dvh] md:max-w-6xl flex flex-col md:flex-row z-10 overflow-hidden shadow-2xl border-white/5">
                ...
                <div className="flex-1 flex flex-col bg-[#0a1a14]">
                    ...
                    <footer className="p-5 md:p-7 border-t border-white/5">
                        ...
                        <button onClick={handleReservation} className="w-full bg-rhum-gold text-rhum-green py-4 font-black uppercase tracking-[0.3em] text-[10px] hover:bg-white transition-all shadow-xl rounded-sm">RÉSERVER DÈS MAINTENANT</button>
                    </footer>
                </div>
            </motion.div>
        </div>,
