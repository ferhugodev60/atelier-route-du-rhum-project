...
export default function Workshops({ onAddToCart }: WorkshopsProps) {
    ...
    const scheduleSlots = ["10h - 11h30", "12h - 13h30", "14h - 15h30", "16h - 17h30", "18h - 19h30"];
                ...
                <div className="mb-20 md:mb-32">
                    ...
                    <div className="flex flex-wrap justify-center md:grid md:grid-cols-5 gap-3 md:gap-4">
                        {scheduleSlots.map((slot, index) => (
                            <motion.div key={index} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} viewport={{ once: true }} className="w-[calc(50%-6px)] md:w-auto group flex flex-col items-center justify-center py-6 px-2 border border-rhum-gold/30 rounded-sm bg-white/[0.03] hover:bg-rhum-gold transition-all duration-500 cursor-default shadow-2xl">
                                <span className="text-rhum-gold/60 text-[8px] uppercase tracking-widest mb-2 group-hover:text-[#0a1a14] transition-colors font-bold">Créneau</span>
                                <span className="text-white font-serif text-base md:text-lg group-hover:text-[#0a1a14] transition-colors font-medium">{slot}</span>
                            </motion.div>
                        ))}
                    </div>
                </div>
