...
interface CartItem {
    cartId: string; // VolumeId ou WorkshopId
    name: string;
    price: number;
    image: string;
    quantity: number;
...
export default function CartDrawer({ isOpen, onClose, items, onRemove }: CartDrawerProps) {
    ...
    /**
     * 🏺 TRANSMISSION : Envoi de la commande à l'alambic (Backend)
     */
    const handleCheckout = async () => {
        ...
        try {
            // Formatage des données pour le contrôleur
            ...
            if (response.status === 201) {
                alert("Votre commande a été scellée ! Redirection vers votre espace...");
                window.location.href = "/mon-compte"; // 🏺 Redirection vers l'historique
            }
        } catch (error: any) {
            console.error("🔥 [CHECKOUT ERROR]:", error.response?.data?.error);
            setOrderError(error.response?.data?.error || "L'alambic a rencontré un problème lors de la commande.");
        } finally {
                    ...
                    <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed right-0 top-0 h-full w-full md:w-[450px] bg-[#0a1a14] z-[101] shadow-2xl border-l border-rhum-gold/10 flex flex-col">
                                        ...
                                        <div className="flex-1 flex flex-col justify-between py-1">
                                            <div>
                                                <div className="flex justify-between items-start">
                                                    <div>
                                                        <span className="text-[7px] uppercase tracking-widest text-rhum-gold/60 mb-1 block">
                                                            {item.workshopId ? 'Expérience' : 'Elixir'}
                                                        </span>
                                                        <h4 className="text-white font-serif text-lg leading-tight">{item.name}</h4>
                                                    </div>
                                                    <button onClick={() => onRemove(item.cartId)} className="text-white/20 hover:text-red-400 transition-colors p-1">
                                                ...
                                                <p className="text-rhum-gold text-[10px] uppercase tracking-widest mt-1">
                                                    {item.workshopId ? `${item.quantity} Participant(s)` : `Qté : ${item.quantity}`}
                                                </p>
                                            </div>
                        ...
                        {items.length > 0 && (
                            <div className="p-8 border-t border-rhum-gold/10 bg-black/20">
                                {orderError && (
                                    <p className="text-red-400 text-[10px] uppercase tracking-widest text-center mb-4 bg-red-400/10 py-2 border border-red-400/20">
                                        {orderError}
                                    </p>
                                )}
                                <div className="flex justify-between items-baseline mb-8">
                                    <span className="text-rhum-cream/40 text-[10px] uppercase tracking-[0.3em]">Total de la commande</span>
                                    <span className="text-3xl font-serif text-rhum-gold">{total}€</span>
                                </div>
                                <button
                                    disabled={isSubmitting}
                                    onClick={handleCheckout}
                                    className={`w-full py-5 font-black uppercase tracking-[0.3em] text-[11px] transition-all rounded-sm shadow-xl ${
                                        isSubmitting ? 'bg-gray-700 text-gray-500 cursor-not-allowed' : 'bg-rhum-gold text-rhum-green hover:bg-white'
                                    }`}
                                >
                                    {isSubmitting ? 'DISTILLATION EN COURS...' : 'CONFIRMER LA COMMANDE'}
                                </button>
