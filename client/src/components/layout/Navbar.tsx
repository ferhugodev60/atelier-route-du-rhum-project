...
export default function Navbar({ cartCount, onOpenCart }: NavbarProps) {
    ...
    // FONCTIONS DE BASCULEMENT (L'un ferme l'autre)
    const openLogin = () => {
    ...
    return (
        <>
                ...
                <div className="max-w-7xl mx-auto px-8 md:px-16 flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-6 md:gap-10">
                        ...
                        <div className="hidden lg:flex items-center gap-10">
                            {NAV_LINKS.map((link) => (
                                link.isExternal
                                    ? <Link key={link.name} to={link.href} className={linkStyle}>
                                        {link.name}
                        ...
                        {/* 4. AUTHENTIFICATION */}
                        {user ? (
                            <button onClick={() => navigate('/mon-compte')} className={`hidden lg:flex items-center px-6 py-2.5 text-[9px] font-black uppercase tracking-[0.2em] transition-all rounded-full border border-rhum-gold/30 hover:border-rhum-gold ${isScrolled ? 'text-white' : 'text-rhum-gold'}`}>
                                Bonjour {user.firstName} !
                            </button>
                        ) : (
                            <button onClick={openLogin} className={`hidden lg:block px-8 py-2.5 text-[9px] font-black uppercase tracking-[0.2em] transition-all rounded-full ${isScrolled ? 'bg-rhum-gold text-rhum-green hover:bg-white shadow-lg' : 'border border-rhum-gold text-rhum-gold hover:bg-rhum-gold hover:text-rhum-green'}`}>
                                SE CONNECTER
                            </button>
                        )}
                    ...
                    {isMobileMenuOpen && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="absolute top-full left-0 w-full bg-rhum-green border-t border-white/5 overflow-hidden lg:hidden">
                            <div className="flex flex-col p-10 gap-8 text-center items-center">
                                ...
                                {user ? (
                                    <button onClick={() => { navigate('/mon-compte'); setIsMobileMenuOpen(false); }} className="w-full max-w-[240px] px-8 py-4 border border-rhum-gold text-rhum-gold text-[10px] font-black uppercase tracking-[0.2em] rounded-full">
                                        Bonjour {user.firstName}
                                    </button>
                                ) : (
                                    <button onClick={() => { openLogin(); setIsMobileMenuOpen(false); }} className="mt-4 w-full max-w-[240px] px-8 py-4 bg-rhum-gold text-rhum-green text-[10px] font-black uppercase tracking-[0.2em] rounded-full">
                                        SE CONNECTER
                                    </button>
                                )}
                            </div>
            ...
            <RegisterModal
                ...
                onSwitchToLogin={openLogin} // Bascule vers la connexion
            />
        </>
    );
}